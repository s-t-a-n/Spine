...
public:
    ...
    constexpr CompositeUnit() = default;
    CompositeUnit(const NumeratorUnit& numerator, const DenominatorUnit& denominator)
        : _numerator(numerator), _denominator(denominator) {}
    CompositeUnit(const CompositeUnit&) = default;
    CompositeUnit& operator=(const CompositeUnit&) = default;
    CompositeUnit(CompositeUnit&& other) noexcept = default;
    CompositeUnit& operator=(CompositeUnit&& other) noexcept = default;

    template<typename OtherNumeratorUnit, typename OtherDenominatorUnit,
             std::enable_if_t<detail::are_interchangeable<NumeratorUnit, DenominatorUnit, OtherNumeratorUnit,
                                                          OtherDenominatorUnit>(),
                              int> = 0>
    CompositeUnit(const CompositeUnit<OtherNumeratorUnit, OtherDenominatorUnit>& other)
    ...
    template<typename OtherNumeratorUnit, typename OtherDenominatorUnit,
             std::enable_if_t<detail::are_interchangeable<NumeratorUnit, DenominatorUnit, OtherNumeratorUnit,
                                                          OtherDenominatorUnit>(),
                              int> = 0>
    CompositeUnit& operator=(const CompositeUnit<OtherNumeratorUnit, OtherDenominatorUnit>& other) {
    ...
    /// Simplify composite by reducing numerator and denominator by greatest common denominator. Provided
    ...
    template<typename OtherNumeratorUnit, typename OtherDenominatorUnit>
    auto operator+(const CompositeUnit<OtherNumeratorUnit, OtherDenominatorUnit>& other) const {
        CompositeUnit result = *this;
        result += other;
        return result;
    }
    ...
    [[nodiscard]] CompositeUnit operator-() const { return CompositeUnit(numerator(), -denominator()); }

    template<typename AT>
    std::enable_if_t<std::is_arithmetic_v<AT>, CompositeUnit> operator*(const AT& scalar) const {
        return CompositeUnit{_numerator * scalar, _denominator};
    }
    template<typename AT>
    std::enable_if_t<std::is_arithmetic_v<AT>, CompositeUnit&> operator*=(const AT& scalar) {
        _numerator *= scalar;
        return *this;
    }
    template<typename AT>
    ...
    template<typename OtherNumeratorUnit, typename OtherDenominatorUnit>
    bool operator==(const CompositeUnit<OtherNumeratorUnit, OtherDenominatorUnit>& other) const {
